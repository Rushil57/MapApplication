...
def buildPLSSLayer(TN,RN,SECT,PK): #,l_qq,MAPTID,qtrqtr):
  ...
  for s in SECT:
    okLayer.selectByExpression('"ABSTRACT_L"=\'' + str(s) + '\' and "LEVEL3_SUR" = \'' + str(TN[f_indx]) + '\' and "LEVEL1_SUR" = \'' + str(RN[f_indx]) + '\'')
    recordCount = recordCount + 1
    file = open("##myTxtProgressFile##","w")
    file.writelines(str(recordCount) + " / ##totalPer##")
    file.close()
    if len(okLayer.selectedFeatures())<1:
      print("Couldn't Find Section:",'"ABSTRACT_L"=\'' + str(s) + '\' and "LEVEL3_SUR" = \'' + str(TN[f_indx]) + '\' and "LEVEL1_SUR" = \'' + str(RN[f_indx]))
      f_indx=f_indx+1
      continue
